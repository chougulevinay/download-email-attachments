...
module.exports = function (state, attachment, mail, callback, errorCb) {
  ...
  attachment.stream.on('end', function () {
    ...
    if (state.attachmentHandler) {
      ...
      let mailBoxData ={uidvalidity: state.uidvalidity, uids:state.uids};
      state.attachmentHandler({mailBoxData, rootPath: state.directory, path: filePath, generatedFileName: generatedFileName, att: attachment, mail: mail}, callback, errorCb)
    } else {
      callback()
    }
  })
